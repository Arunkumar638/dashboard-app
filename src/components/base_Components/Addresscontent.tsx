...
const Address = () => {
  ...
  return (
    <div>
      <main>
        <h2 className={styles.headertext3}>User Address</h2>
        <br />
        <br />
        <Button onClick={handleAdd} type='primary' danger className='font-serif h-button ml-profile text-base'>
          + Add new Address
        </Button>
        <div>
          {isTextareaVisible
            ? isTextarea.map((address, index) => (
      ...
      <AddressPopupForm trigger={isPopupVisible}>
        ...
        <div id='option'>
          <select
            value={IsEditPopup ? address.country : selectCountry}
