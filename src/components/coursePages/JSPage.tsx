...
import { createFromIconfontCN, AuditOutlined } from '@ant-design/icons';
import type { MenuProps } from 'antd';
import Editor from '@monaco-editor/react';
import styles from '../../styles/sidebar.module.css';
import { Card } from 'antd';
import compileCode from '../../actions/compilerActions';
import JSTopicList from '../../Content/JS.json'
import LoadJSContent from '../LoadJSContent';
...
const JS: React.FC = () => {
  ...
  const defaultId = 'javascript_introduction';
  ...
  return (
    ...
    <Sider collapsible collapsed={collapsed} onCollapse={(value) => setCollapsed(value)} width={300} style={{height:'100vh'}}>
        ...
        theme='dark'
        defaultSelectedKeys={['javascript_introduction']}
        style={{height:'840px'}}
        defaultOpenKeys={['sub1']}
        mode="inline"
        items={JSTopicList}
        className={`bg-dark-purple text-white font-bold text-l ${collapsed?'bg-dark-purple hidden':null} transition-all duration-1000 overflow-y-scroll`}
        />
      </Sider>
      ...
      <Header style={{ padding: 0}} className='bg-gradient-to-r from-orange to-coral shadow-lg'>
        <Button type="primary" icon={<ArrowLeftOutlined />} className='ml-top bg-blue' onClick={route}>Dashboard</Button>
          ...
          <div className={styles.position}>
            ...
            <Card title="JS Editor" bordered={true} className={`${collapsed?"w-card-expand":"w-card-collapse"} transition-all duration-500 bg-orange h-card`}>
            <FloatButton
             shape="square"
             type="primary"
            ...
            >
            {defaultmode && <LoadJSContent id={defaultId}/>}  
            {!!id && <LoadJSContent id={id}/>}
          </Card> 
      </Layout>
    </Layout>
  );
