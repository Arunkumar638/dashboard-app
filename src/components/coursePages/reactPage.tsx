...
import reactTopicList from '../../Content/React.json'
import LoadReactContent from '../LoadReactContent';
...
const ReactPage: React.FC = () => {
  ...
  const defaultId = 'react_react_introduction';
  ...
  return (
    ...
    <Sider collapsible collapsed={collapsed} onCollapse={(value) => setCollapsed(value)} width={300} style={{height:'100vh'}}>
        ...
        theme='dark'
        defaultSelectedKeys={['react_react_introduction']}
        style={{height:'840px'}}
        defaultOpenKeys={['sub1']}
        mode="inline"
        items={reactTopicList}
      ...
      <Header style={{ padding: 0}} className='bg-gradient-to-r from-orange to-coral shadow-lg'>
        <Button type="primary" icon={<ArrowLeftOutlined />} className='ml-top bg-blue' onClick={route}>Dashboard</Button>
          ...
          <div className={styles.position}>
            ...
            <Card title="React JS Editor" bordered={true} className={`${collapsed?"w-card-expand":"w-card-collapse"} transition-all duration-500 bg-orange h-card`}>
            <FloatButton
             shape="square"
             type="primary"
            ...
            >
            {defaultmode && <LoadReactContent id={defaultId}/>}  
            {!!id && <LoadReactContent id={id}/>}
          </Card>          
      </Layout>
    </Layout>
  );
